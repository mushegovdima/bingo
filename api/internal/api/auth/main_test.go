...
func (f *fakeAuthenticator) AuthenticateUser(ctx context.Context, req *authservice.UserLoginByTelegramRequest) (*authservice.AuthResult, error) {
	...
	return &authservice.AuthResult{
		SessionID: 1,
		UserID:    42,
		User:      &userdomain.User{ID: 42, Name: "Test User"},
	}, nil
}

func (f *fakeAuthenticator) AuthenticateWebApp(_ context.Context, _ *authservice.WebAppLoginRequest) (*authservice.AuthResult, error) {
	return nil, nil
}

type fakeUserSvc struct {
	getByIDFn func(ctx context.Context, id int64) (*userdomain.User, error)
}

func (f *fakeUserSvc) GetById(ctx context.Context, id int64) (*userdomain.User, error) {
	if f.getByIDFn != nil {
...
func (f *fakeSessionCreator) CreateSession(_ context.Context, _ sessioncontract.CreateInput) (int64, error) {
	return 1, nil
}
			...
			&fakeAuthenticator{
				authenticateFn: func(_ context.Context, _ *authservice.UserLoginByTelegramRequest) (*authservice.AuthResult, error) {
					return &authservice.AuthResult{
						SessionID: 1,
						UserID:    42,
						User:      &userdomain.User{ID: 42, Name: "Alice"},
					}, nil
				},
			},
			&fakeUserSvc{},
...
func TestAuthHandler_GetMe(t *testing.T) {
	t.Run("returns 200 with current user", func(t *testing.T) {
		handler := newAuthHandler(
			&fakeAuthenticator{},
			&fakeUserSvc{
				getByIDFn: func(_ context.Context, id int64) (*userdomain.User, error) {
					return &userdomain.User{ID: id, Name: "Bob"}, nil
				},
			},
		)
	...
	t.Run("returns 404 when user not found", func(t *testing.T) {
		handler := newAuthHandler(
			&fakeAuthenticator{},
			&fakeUserSvc{
				getByIDFn: func(_ context.Context, _ int64) (*userdomain.User, error) { return nil, nil },
			},
		)
	...
	t.Run("returns 500 when user service fails", func(t *testing.T) {
		handler := newAuthHandler(
			&fakeAuthenticator{},
			&fakeUserSvc{
				getByIDFn: func(_ context.Context, _ int64) (*userdomain.User, error) {
