...
import (
	...
	templateapi "go.mod/internal/api/template"
...
func NewRouter(svc *services.Container, cfg *config.Config, store *sessions.CookieStore, logger *slog.Logger) *chi.Mux {
	...
	r.Use(chimw.Throttle(cfg.ThrottleLimit))
	...
	managerOnly := apimiddleware.RequireRole(svc.User, userdomain.Manager)
	...
	submissionHandler := submissionapi.NewHandler(svc.Submission, svc.User, logger)
	...
	templateHandler := templateapi.NewHandler(svc.Template, logger)
	...
	r.Route("/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Mount("/templates", templateHandler.Routes(managerOnly))
		})
